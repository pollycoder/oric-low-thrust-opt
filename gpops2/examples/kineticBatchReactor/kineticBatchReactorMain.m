...
auxdata.phase(1).k1hat      = k1hat/chr2sec;
auxdata.phase(1).kminus1hat = kminus1hat/chr2sec;
auxdata.phase(1).k2hat      = k2hat/chr2sec;
auxdata.phase(1).a          = a/chr2sec^2;
auxdata.phase(2).k1hat      = k1hat/chr2min;
auxdata.phase(2).kminus1hat = kminus1hat/chr2min;
auxdata.phase(2).k2hat      = k2hat/chr2min;
auxdata.phase(2).a          = a/chr2min^2;
auxdata.phase(3).k1hat      = k1hat/chr2min;
auxdata.phase(3).kminus1hat = kminus1hat/chr2min;
auxdata.phase(3).k2hat      = k2hat/chr2min;
auxdata.phase(3).a          = a/chr2min^2;
...
%----------------------------------------%
% Bounds and Guess in Phase 1 of Problem %
%----------------------------------------%
iphase = 1;
Numsec = 1;
meshphase(iphase).colpoints = 15.*ones(1,Numsec);
meshphase(iphase).fraction   = 1/Numsec.*ones(1,Numsec);
bounds.phase(iphase).initialtime.lower = t10sec;
bounds.phase(iphase).initialtime.upper = t10sec;
bounds.phase(iphase).finaltime.lower   = t1fsec;
bounds.phase(iphase).finaltime.upper   = t1fsec;
...
%----------------------------------------%
% Bounds and Guess in Phase 2 of Problem %
%----------------------------------------%
iphase = 2;
Numsec = 1;
meshphase(iphase).colpoints = 15.*ones(1,Numsec);
meshphase(iphase).fraction   = 1/Numsec.*ones(1,Numsec);
bounds.phase(iphase).initialtime.lower = t20lowmin;
bounds.phase(iphase).initialtime.upper = t20uppmin;
bounds.phase(iphase).finaltime.lower   = t2flowmin;
bounds.phase(iphase).finaltime.upper   = t2fuppmin;
...
%----------------------------------------%
% Bounds and Guess in Phase 3 of Problem %
%----------------------------------------%
iphase = 3;
Numsec = 1;
meshphase(iphase).colpoints = 15.*ones(1,Numsec);
...
y4fmin = 1;
y4fmax = y4max;
bounds.phase(iphase).initialtime.lower = t30lowmin;
bounds.phase(iphase).initialtime.upper = t30uppmin;
bounds.phase(iphase).finaltime.lower   = t3flowmin;
...
bounds.phase(iphase).state.lower = [y1min,y2min,y3min,y4min,y5min,y6min];
bounds.phase(iphase).state.upper = [y1max,y2max,y3max,y4max,y5max,y6max];
%bounds.phase(iphase).finalstate.lower = [y1min,y2min,y3min,y4fmin,y5min,y6min];
%bounds.phase(iphase).finalstate.upper = [y1max,y2max,y3max,y4fmax,y5max,y6max];
bounds.phase(iphase).finalstate.lower = [y1min,y2min,y3min,y4min,y5min,y6min];
bounds.phase(iphase).finalstate.upper = [y1max,y2max,y3max,y4max,y5max,y6max];
bounds.phase(iphase).control.lower = [u1min,u2min,u3min,u4min,u5min];
bounds.phase(iphase).control.upper = [u1max,u2max,u3max,u4max,u5max];
