function [grdjacpat, probinfo] = gpopsGrdJacPatRPMD(probinfo)

% gpopsGrdJacPatRPMD
% this function finds the sparsity pattern of the combined gradient and
% non linear part of the Jacobian, where the gradient is the first row
...
% add 1 to rows of Jacobian, to account for adding gradient
jacnonlin(:,1) = jacnonlin(:,1) + 1;

% stack gradient and Jacobian
grdjacpat = [grdpat; jacnonlin];

% number of nonlinear nonzeros in combined gradient and Jacobian
probinfo.grdnnz = grdnnz;
probinfo.jacnnz = size(jacnonlin,1);