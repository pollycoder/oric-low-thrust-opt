function contfirstder = gpopsContFirstFD(input, probinfo)
    ...
    for phasecount = 1:numphase;
        ...
        if nvccount <= contnvc(phasecount);
            ...
            for dynamicscount = 1:numstatep;
                ...
                if refmark ~= 0;
                    contfirstder(phasecount).dynamicsgrd(:,refmark) = (pertoutput(phasecount).dynamics(:,dynamicscount) - output(phasecount).dynamics(:,dynamicscount))./hstep(phasecount).h1;
                end
            end
            ...
            for pathcount = 1:numpathp;
                ...
                if refmark ~= 0;
                    contfirstder(phasecount).pathgrd(:,refmark) = (pertoutput(phasecount).path(:,pathcount) - output(phasecount).path(:,pathcount))./hstep(phasecount).h1;
                end
            end
            ...
            for intergralcount = 1:numintegralp;
                ...
                if refmark ~= 0;
                    contfirstder(phasecount).integrandgrd(:,refmark) = (pertoutput(phasecount).integrand(:,intergralcount) - output(phasecount).integrand(:,intergralcount))./hstep(phasecount).h1;
                end
            end
        end
    end
