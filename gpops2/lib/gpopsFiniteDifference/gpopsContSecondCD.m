function contsecondder = gpopsContSecondCD(input, probinfo)
...
for nvccount = 1:maxnvc;
    ...
    for phasecount = 1:numphase;
        ...
        if nvccount <= contnvc(1,phasecount);
            ...
            if varnum1 <= numstatep;
                ...
                pertinput1.phase(phasecount).state(:,refmark) = pertinput1.phase(phasecount).state(:,refmark) + h./2;
                pertinput2.phase(phasecount).state(:,refmark) = pertinput2.phase(phasecount).state(:,refmark) - h./2;
                pertinput3.phase(phasecount).state(:,refmark) = pertinput3.phase(phasecount).state(:,refmark) + h./2;
                pertinput4.phase(phasecount).state(:,refmark) = pertinput4.phase(phasecount).state(:,refmark) - h./2;
            elseif varnum1 <= numstatep+numcontrolp;
                % perturb control
                refmark = varnum1-numstatep;
                h = ustep.*(abs(input.phase(phasecount).control(:,refmark))+1);
                hstep(phasecount).h1 = h;
                pertinput1.phase(phasecount).control(:,refmark) = pertinput1.phase(phasecount).control(:,refmark) + h./2;
                pertinput2.phase(phasecount).control(:,refmark) = pertinput2.phase(phasecount).control(:,refmark) - h./2;
                pertinput3.phase(phasecount).control(:,refmark) = pertinput3.phase(phasecount).control(:,refmark) + h./2;
                pertinput4.phase(phasecount).control(:,refmark) = pertinput4.phase(phasecount).control(:,refmark) - h./2;
            elseif varnum1 == numstatep+numcontrolp+1;
                % perturb time
                h = ustep.*(abs(input.phase(phasecount).time)+1);
                hstep(phasecount).h1 = h;
                pertinput1.phase(phasecount).time = pertinput1.phase(phasecount).time + h./2;
                pertinput2.phase(phasecount).time = pertinput2.phase(phasecount).time - h./2;
                pertinput3.phase(phasecount).time = pertinput3.phase(phasecount).time + h./2;
                pertinput4.phase(phasecount).time = pertinput4.phase(phasecount).time - h./2;
            elseif varnum1 <= numstatep+numcontrolp+1+numparameters;
                ...
                hstep(phasecount).h1 = h;
                pertinput1.phase(phasecount).parameter(:,refmark) = pertinput1.phase(phasecount).parameter(:,refmark) + h./2;
                pertinput2.phase(phasecount).parameter(:,refmark) = pertinput2.phase(phasecount).parameter(:,refmark) - h./2;
                pertinput3.phase(phasecount).parameter(:,refmark) = pertinput3.phase(phasecount).parameter(:,refmark) + h./2;
                pertinput4.phase(phasecount).parameter(:,refmark) = pertinput4.phase(phasecount).parameter(:,refmark) - h./2;
            end
    ...
    pertoutput4 = feval(probinfo.contfunction, pertinput4);
    ...
    for phasecount = 1:numphase;
        ...
        if nvccount <= contnvc(1,phasecount);
            ...
            for dynamicscount = 1:numstatep;
                ...
                if refmark ~= 0;
                    contsecondder(phasecount).dynamicshes(:,refmark) = (pertoutput1(phasecount).dynamics(:,dynamicscount) - pertoutput2(phasecount).dynamics(:,dynamicscount) - pertoutput3(phasecount).dynamics(:,dynamicscount) + pertoutput4(phasecount).dynamics(:,dynamicscount))./(hstep(phasecount).h1.*hstep(phasecount).h2);
                end
            end
            ...
            for pathcount = 1:numpathp;
                ...
                if refmark ~= 0;
                    contsecondder(phasecount).pathhes(:,refmark) = (pertoutput1(phasecount).path(:,pathcount) - pertoutput2(phasecount).path(:,pathcount) - pertoutput3(phasecount).path(:,pathcount) + pertoutput4(phasecount).path(:,pathcount))./(hstep(phasecount).h1.*hstep(phasecount).h2);
                end
            end
            ...
            for intergralcount = 1:numintegralp;
                ...
                if refmark ~= 0;
                    contsecondder(phasecount).integrandhes(:,refmark) = (pertoutput1(phasecount).integrand(:,intergralcount) - pertoutput2(phasecount).integrand(:,intergralcount) - pertoutput3(phasecount).integrand(:,intergralcount) + pertoutput4(phasecount).integrand(:,intergralcount))./(hstep(phasecount).h1.*hstep(phasecount).h2);
                end
            end
        end
    end
