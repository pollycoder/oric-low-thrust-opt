function [conthessamples, endphessamples] = gpopsRandomHes(setup, probinfo, numsamples)
...
if probinfo.analyticflag;
    conthessamples = feval(probinfo.conthes, continput);
else
    conthessamples = feval(probinfo.conthes, continput, probinfo);
end
% -------------------------------------------------------------------------
...
objhessamples = zeros(numsamples,probinfo.derivativemap.objnnz2);
if numeventgroup ~= 0;
    eventnnz2 = probinfo.derivativemap.eventnnz2;
    eventgroup(numeventgroup).eventhes = [];
    for eventgroupcount = 1:numeventgroup;
        eventgroup(eventgroupcount).eventhes = zeros(numsamples,eventnnz2(eventgroupcount));
    end
end
...
for samplecount = 1:numsamples;
    ...
    if probinfo.analyticflag;
        endpsecondder = feval(probinfo.endphes, endpinput);
    else
        endpsecondder = feval(probinfo.endphes, endpinput, probinfo);
    end
    
    % store derivative values of objective for current sample
    objhessamples(samplecount,:) = endpsecondder.objectivehes;
    
    % store derivative values of events for current sample
    if numeventgroup ~= 0;
        for eventgroupcount = 1:numeventgroup;
            eventgroup(eventgroupcount).eventhes(samplecount,:) = endpsecondder.eventgroup(eventgroupcount).eventhes;
        end
    end
end
endphessamples.objectivehes = objhessamples;
if numeventgroup ~= 0;
    endphessamples.eventgroup = eventgroup;
end