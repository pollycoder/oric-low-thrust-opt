function [continput, endpinput] = gpopsInputFromInterpSolution(solution, setup, numsamples)

% gpopsInputFromInterpSolution
% this function generates the inputs for the continuous and endpoint
% functions from the solution interpolated to a time grid of numsamples
...
for phasecount = 1:numphase;
    ...
    % get solution time
    guesstime = solphase.time;
    interptime = linspace(guesstime(1), guesstime(end), numsamples)';
    
    % get input for state solution
    contphase(phasecount).state = interp1(guesstime, solphase.state, interptime, 'pchip');
    ...
    if controlswitch;
        if ~isempty(solphase.control);
            contphase(phasecount).control = interp1(guesstime, solphase.control, interptime, 'pchip');
        end
    end
    ...
    if numparameter ~= 0;
        contphase(phasecount).parameter = ones(numsamples,1)*solution.parameter;
    end
