function grdjacnz = gpopsGrdJacnzRPMI(Z, probinfo)
...
grdjacnz = zeros(probinfo.jacnonlinnnz + probinfo.grdnnz,1);
        ...
        if varnum <= numstatep+numcontrolp;
            ...
            for dynamicscount = 1:numstatep;
                ...
                if dynamicsgrdref ~= 0;
                    ...
                    grdjacnz(jacindex) = -tdiff*E(:,4).*contgrdp.dynamicsgrd(E(:,2),dynamicsgrdref);
                end
            end
        ...
        elseif varnum <= numstatep+numcontrolp+1+numparameter;
            ...
            for dynamicscount = 1:numstatep;
                ...
                if dynamicsgrdref ~= 0;
                    ...
                    % assign values
                    grdjacnz(jacindex) = -tdiff*full(Emat*contgrdp.dynamicsgrd(:,dynamicsgrdref));
                end
            end
            % derivatives of path constraints
            if numpathp ~= 0;
    ...
    if phasetimeflag;
        ...
        for dynamicscount = 1:numstatep;
            ...
            if dynamicsgrdref ~= 0;
                ...
                jacindex = jacmarkers:jacmarkere;
                % assign values
                grdjacnz(jacindex) = full(Emat*(contfunp.dynamics(:,dynamicscount)/2 - tdiff*dtdt0.*contgrdp.dynamicsgrd(:,dynamicsgrdref)));
                ...
                jacindex = jacmarkers:jacmarkere;
                % assign values
                grdjacnz(jacindex) = full(Emat*(-contfunp.dynamics(:,dynamicscount)/2 - tdiff*dtdtf.*contgrdp.dynamicsgrd(:,dynamicsgrdref)));
            else
                % derivative respect to t0
                jacmarkers = jacmarkere + 1;
                jacmarkere = jacmarkere + numnodesp;
        ...
        for dynamicscount = 1:numstatep;
            ...
            jacindex = jacmarkers:jacmarkere;
            % assign values
            grdjacnz(jacindex) = full(Emat*contfunp.dynamics(:,dynamicscount))/2;
