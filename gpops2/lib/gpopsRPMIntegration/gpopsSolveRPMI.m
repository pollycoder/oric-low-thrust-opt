function result = gpopsSolveRPMI(setup, probinfo)
...
disp(' Finding Radau Points, Integration Weights');
disp(' Integration Matrix, and Initial Value Matrix');
...
[probinfo, ocpscales] = gpopsScaleRPMI(setup, probinfo);

% call NLP
if strcmpi(setup.nlp.solver,'snopt');
    result = gpopsSnoptHandlerRPMI(ZG, ZL, ZU, FL, FU, setup.name, setup.nlp.options, probinfo);
elseif strcmpi(setup.nlp.solver,'ipopt');
    result = gpopsIpoptHandlerRPMI(ZG, ZL, ZU, FL, FU, setup.name, setup.nlp.options, probinfo);
end
